Vagrant.configure("2") do |config|
    ...
    config.vm.hostname = "regina.test"
    ...
    config.vm.synced_folder ".", "/var/www/app", :mount_options => ["dmode=775", "fmode=777"]
    config.vm.provision "shell", inline: <<-SHELL
      # # User Defined Settings # #
        ...
        XDEBUG_MAX_DEPTH="3"
        INSTALL_DB="yes" # yes or no
        DB_IS_MAGENTO="no" # 1, 2 or no
        DB_IS_WORDPRESS="no" # yes or no
        DB_NAME="cms"
        DB_USER="tester"
        DB_PASS="s12e3d232r23e2"
        #DB_PREFIX=""
        DB_FILENAME="init.sql" # located below document root
        ...
        echo "+  192.168.33.113   ${FULL_DOMAIN}"
        echo "+                                                          +"
